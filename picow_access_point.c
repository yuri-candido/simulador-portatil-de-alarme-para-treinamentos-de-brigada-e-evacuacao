...
#define LED_TEST_BODY "<html><body><h1>Hello from Pico.</h1><p>Clique para %s o alarme</p><p><a href=\"?led=%d\">Clique para %s o alarme</a></body></html>"
...
uint8_t ssd[1024]; // ou defina o tamanho correto, por exemplo: ssd1306_buffer_length
...
static int test_server_content(const char *request, const char *params, char *result, size_t max_result_len) {
    ...
    if (strncmp(request, LED_TEST, sizeof(LED_TEST) - 1) == 0) {
        ...
        if (params) {
            ...
            if (led_param == 1) {
                if (led_state) {
                    ...
                    led_blinking = false; // Para de piscar
                    cyw43_gpio_set(&cyw43_state, LED_GPIO, false);

                    // Alarme desligado: mostra "Alarme desligado"
                    char *text[] = {
                        "  Alarme desligado\n",
                        };
        ...
        // Generate result
        if (led_state) {
            len = snprintf(result, max_result_len, LED_TEST_BODY, "LIGAR", 0, "DESLIGAR");
        } else {
            len = snprintf(result, max_result_len, LED_TEST_BODY, "DESLIGAR", 1, "LIGAR");
        }
    }
    return len;
}
...
int main() {
    ...
    gpio_put(BUZZER_PIN, 0); // Garante que inicie desligado
    ...
    // Loop cooperativo para piscar o LED
    ...
    if (led_blinking) {
        if (absolute_time_diff_us(get_absolute_time(), next_blink_time) <= 0) {
            led_on = !led_on;
            cyw43_gpio_set(&cyw43_state, LED_GPIO, led_on);
            next_blink_time = make_timeout_time_ms(300); // pisca a cada 300 ms
        }
        char *text[] = {
            "  Evacuar\n",
           };
